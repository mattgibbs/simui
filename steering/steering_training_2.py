...
class SteeringDisplay(Display):
    ...
    def initialize_orbit(self):
        ...
        bpm_list = []
        for sector in ("LI21", "LI22", "LI23", "LI24", "LI25", "LI26", "LI27", "LI28", "LI29", "LI30"):
            for unit in (201, 301, 401, 501, 601, 701, 801, 901):
                bpm_list.append("BPMS:{}:{}".format(sector, unit))
        bpm_list = bpm_list[:-1] #Get rid of "BPMS:LI30:901", which doesn't exist.
        orbit = Orbit(bpm_list, auto_connect=False, parent=self)
        self.x_magnet_list = MagnetList("X", "xcor_list_training_2.json", parent=self)
